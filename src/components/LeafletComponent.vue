<template>
  <span
    ><l-map ref="map" :center="center" :zoom="zoom" :options="mapOptions" style="height: 400px">
      <!-- Modal -->
      <div
        class="modal fade"
        id="selectAreaModal"
        tabindex="-1"
        role="dialog"
        aria-labelledby="Select an area"
        aria-hidden="true"
      >
      ...
      <l-control-fullscreen
        :options="{ title: { false: 'Full Screen', true: 'Be regular' } }"
        position="topleft"
      />
      ...
      <l-circle-marker v-for="(item, index) in locations" v-bind:key="index"
      :lat-lng="item.latLng"
      :radius="6"
      ...
      :color="borderColor"
          ...
          :opacity="0.3"
    />
      <!--                <l-marker :lat-lng="markerLatLng[1]" :icon="icon"/>-->
      <!--                <l-marker :lat-lng="markerLatLng[1]"></l-marker>-->
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <select v-model="selectedArea" class="form-select" aria-label="Default select example">
              <option value="-1" selected>Select an option</option>
              <option v-for="(item, index) in this.$store.state.settings.predifinedMapAreas" :key="item.id" :value="index">{{item.title}}</option>
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" @click="insertMapFilter">Insert</button>
          </div>
        </div>
      </div>
    </div> -->
